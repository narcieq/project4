...
int main(int argc, char* argv[])
{
  ...
  double w[17], average[6], total_average[6],acpt_configs,
  ...
  n_spins = atoi(argv[2]);
  mcs = atoi(argv[3]);
  initial_temp =(double) atof(argv[4]);
  final_temp = (double) atof(argv[5]);
  temp_step =(double) atof(argv[6]);
  ...
  */
  ...
  for ( double temperature = initial_temp; temperature <= final_temp; temperature+=temp_step){
    ...
    E = M = 0.;
    // initailise number of accepted configurations
    acpt_configs=0.;
    ...
    for (int cycles = myloop_begin; cycles <= myloop_end; cycles++){
      Metropolis(n_spins, idum, spin_matrix,acpt_configs, E, M, w);
      // update expectation values  for local node
      //acpt_configs+=acpt_configs;
      average[0] += E;    average[1] += E*E;
      average[2] += M;    average[3] += M*M; average[4] += fabs(M);
      average[5] = acpt_configs;
...
// Metropolis algo
void Metropolis(int n_spins, long& idum, int **spin_matrix,double& acpt_configs,double& E, double& M, double *w)
{
  // loop over all spins
  for(int y =0; y < n_spins; y++) {
    for (int x= 0; x < n_spins; x++){
...
void output(int n_spins, int mcs, double temperature, double *total_average)
{
  ...
  ofile << setw(15) << setprecision(8) << total_average[5]<<endl;
  cout<<"accepted configuration for Mcs "<<mcs<<" is: "<<total_average[5]<<endl;
} // end output function
